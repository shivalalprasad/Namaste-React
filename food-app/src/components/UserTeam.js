...
import { GIT_HUB_USER } from "../util/constants";
...
class UserTeam extends React.Component {
  ...
  render() {
    const { name, role, info, avatar, github, leetcode, username } = this.props;
    class Github extends UserTeam {
      render() {
        return (
    ...
    class LeeetCode extends UserTeam {
      render() {
        return (
    ...
    return (
      <div className="p-4 lg:w-1/2 hover:bg-gray-100 rounded hover:rounded-full cursor-pointer" onClick={() => {
        window.open(GIT_HUB_USER + username)
      }}>
        <div className="h-full flex sm:flex-row flex-col items-center sm:justify-start justify-center text-center sm:text-left">
          <img alt="team" className="flex-shrink-0 rounded-full w-48 h-48 object-cover object-center sm:mb-0 mb-4" src={avatar} />
